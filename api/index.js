...
(function(){
  ...
  window.fetch=async function(r,i){let u=r instanceof Request?r.url:r,m=(i?.method||(r instanceof Request?r.method:'GET')).toUpperCase();if(typeof u==='string'&&u.includes(window.__PXS.loggerUrl))return oF.apply(this,arguments);send('network_request',{url:u,method:m,type:'fetch'});let f=u,p=0;try{const o=new URL(u,location.href),h=location.hostname;if(o.hostname!==h&&o.protocol.startsWith('http')){f=window.__PXS.assetProxy+'?url='+encodeURIComponent(u);p=1;arguments[0]=f}}catch(e){}try{const res=await oF.apply(this,arguments);let b='[unreadable]';try{const c=res.clone(),t=await c.text();b=t.length>5e4?t.substring(0,5e4)+'\\n...[truncated]':t}catch(e){}buf(u,m,b);if(typeof u==='string'&&!/\\.(css|js|png|jpg|jpeg|gif|svg|woff2?|ttf|ico)$/i.test(u))window.__PXS.lastRequest={url:u,method:m,data:b};return res}catch(err){buf(u,m,'[Error:'+err.message+']');throw err}};
  window.XMLHttpRequest=function(){const x=new oX();let xm='GET',xu='';const oo=x.open;x.open=function(m,u,...r){xm=(m||'GET').toUpperCase();xu=typeof u==='string'?u:String(u);return oo.apply(this,[m,u,...r])};x.addEventListener('load',function(){if(typeof xu==='string'&&xu.includes(window.__PXS.loggerUrl))return;let b='[binary]';try{if(!x.responseType||x.responseType==='text')b=x.responseText;else if(x.responseType==='json')b=JSON.stringify(x.response)}catch(e){}buf(xu,xm,b);if(typeof xu==='string'&&!/\\.(css|js|png|jpg|jpeg|gif|svg|woff2?|ttf|ico)$/i.test(xu)){let a=xu;try{a=new URL(xu,document.baseURI).href}catch(e){}window.__PXS.lastRequest={url:a,method:xm,data:b}}});x.addEventListener('error',function(){buf(xu,xm,'[XHR Error]')});return x};
...
function processHtml(html, targetUrl, currentProxyUrl, assetProxyBase, fullProxy) {
  ...
  if (fullProxy) {
    // Simple asset rewrite
    $('img, script, link[rel="stylesheet"], iframe, video, source, embed').each((_, el) => {
...
<div class="info-box"><strong>ℹ️ Info:</strong><br>Versi Node.js single-file. Siap deploy ke Vercel.</div></div>
...
app.get('/asset-proxy', async (req, res) => {
  ...
  try {
    const response = await axios({
      ...
      headers: { 'User-Agent': USER_AGENT, 'Accept': req.headers.accept || '*/*', 'Referer': req.headers.referer || '' },
      timeout: 30000, responseType: 'arraybuffer', maxRedirects: 10, validateStatus: () => true
    });
